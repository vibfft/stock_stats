...
class Stock(object):
    ...
    def calculate_base(self, data):
        ...
        for i, each_data in enumerate(data):
            ...
            if i == 0:
                ...
                yester_high = self.percent_change(
                    prev_close, each_data['High'])
    ...
    def calculate_CMF(self, data):
        ...
        for i, each_data in enumerate(data):
            ...
            numerator = self.change(
                each_data['Close'], each_data['Low']) - self.change(each_data['High'],  each_data['Close'])
    ...
    def calculate_RSI(self, data):
        ...
        for i, each_data in enumerate(data):
            ...
            if i > period and diff > 0:
                ...
                # since diff is positive which is going down
                up = (prev_up*float(period - 1))/float(period)

            if i > period and diff < 0:
                up = (prev_up*float(period - 1) + abs(diff))/float(period)
                # since diff is negative which is going up
                down = (prev_down*float(period - 1))/float(period)
    ...
    def generate_footer(self):
        ...
        if self.summary['FiftydayMovingAverage'] is not None:
            print(
                '50-Day: {0:<5}'.format(round(float(self.summary['FiftydayMovingAverage']), 2)))
        if self.summary['TwoHundreddayMovingAverage'] is not None:
            print(
                '200-Day: {0:<5}'.format(round(float(self.summary['TwoHundreddayMovingAverage']), 2)))
        if self.summary['AverageDailyVolume'] is not None:
            print('Avg Vol: {0:<5}'.format(
                round(float(self.summary['AverageDailyVolume']), 2)))
        ...
        if self.summary['OneyrTargetPrice'] is not None:
            print('1 YR Target Price: {0:<5}'.format(
                round(float(self.summary['OneyrTargetPrice']), 2)))
        ...
        if self.summary['MarketCapitalization'] is not None:
            print('MarketCap: {0:<5}'.format(
                self.summary['MarketCapitalization']))
        print
        ...
        if self.summary['PriceBook'] is not None:
            print('PriceBook: {0:<5}'.format(
                round(float(self.summary['PriceBook']), 2)))
        if self.summary['BookValue'] is not None:
            print('BookValue: {0:<5}'.format(
                round(float(self.summary['BookValue']), 2)))
        if self.summary['PriceSales'] is not None:
            print('PriceSales: {0:<5}'.format(
                round(float(self.summary['PriceSales']), 2)))
        if self.summary['ShortRatio'] is not None:
            print('ShortRatio: {0:<5}'.format(
                round(float(self.summary['ShortRatio']), 2)))
        if self.summary['DividendYield'] is not None:
            print('DividendYield: {0:<5}'.format(
                round(float(self.summary['DividendYield']), 2)))
        if self.summary['DividendPayDate'] is not None:
            print('DividendPayDate: {0:<5}'.format(
                self.summary['DividendPayDate']))
        if self.summary['EarningsShare'] is not None:
            print('EarningsShare: {0:<5}'.format(
                round(float(self.summary['EarningsShare']), 2)))

        print
        if self.summary['EPSEstimateNextQuarter'] is not None:
            print('EPS Estimate Next QTR: {0:<5}'.format(
                round(float(self.summary['EPSEstimateNextQuarter']), 2)))
        if self.summary['EPSEstimateCurrentYear'] is not None:
            print('EPS Estimate Current YR: {0:<5}'.format(
                self.summary['EPSEstimateCurrentYear']))
        if self.summary['EPSEstimateNextYear'] is not None:
            print('EPS Estimate Next YR: {0:<5}'.format(
                round(float(self.summary['EPSEstimateNextYear']), 2)))
        print(TColor.ENDC)
    ...
    def print_data(self, year, month, day, each_data, SMA_20, SMA_50, SMA_200, RSI, CMF):
        ...
        p1 = Stock.week_days[date(int(year), int(
            month), int(day)).weekday()]  # day
        p2 = '-'.join([str(each_data[0].year),
                      str(each_data[0].month), str(each_data[0].day)])  # date
        p3 = each_data[1]  # open
    ...
    def display(self, weekday, each_data, SMA_20, SMA_50, SMA_200, RSI, CMF):
        ...
        if weekday == 'ALL':
            self.print_data(year, month, day, each_data,
                            SMA_20, SMA_50, SMA_200, RSI, CMF)
...
def main():
    ...
    if len(sys.argv) <= 3 or len(sys.argv) >= 6:
        print(f"{sys.argv[0]} <stock_symbol> <date_from> <date_to> [ day ]")
        print(f"e.g. {sys.argv[0]} AA 2015-12-01 2016-05-01 FRI")
        sys.exit(1)
    ...
    pdr.IEX_TOKEN = os.environ['IEX_API_KEY']
    ...
    chunksize = 25
    raw_data = pdr.iex.daily.IEXDailyReader(
        symbol.upper(), start, end, chunksize)
    ...
    for i, each_list in enumerate(h.total):
        ...
        if i < 14:
            ...
            h.display(weekday, each_list,
                      h.SMA_20[i - 20], 0, 0, h.RSI[i - 14], h.CMF[i - 20])
        elif i >= 50 and i < 200:
            h.display(weekday, each_list,
                      h.SMA_20[i - 20], h.SMA_50[i - 50], 0, h.RSI[i - 14], h.CMF[i - 20])
        elif i >= 200:
            h.display(weekday, each_list, h.SMA_20[i - 20], h.SMA_50[i - 50],
                      h.SMA_200[i - 200], h.RSI[i - 14], h.CMF[i - 20])

    h.generate_header()
    print(TColor.BLUE)
    print(f"STOCK: {''.join([c.capitalize() for c in symbol])}")
    print(TColor.ENDC)
    # h.generate_footer()
